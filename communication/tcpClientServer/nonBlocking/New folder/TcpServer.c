...
int main(void)
{
	...
	/*open socket*/
	if(FAIL == (serverSocket = socket(AF_INET, SOCK_STREAM, 0)))
	{
		HANDLE_ERROR;
	}

	if(FAIL == setsockopt(serverSocket, SOL_SOCKET ,SO_REUSEADDR, &optval , (socklen_t)sizeof(optval)))
	{
		HANDLE_ERROR;	
	}
	...
	/*bind*/
	if(FAIL == bind(serverSocket, (struct sockaddr*)&serverAddr, sizeof(serverAddr)))
	{
		HANDLE_ERROR;
	}

	/*listen*/
	if(FAIL == listen(serverSocket, NUMBER_OF_CLIENTS))
	{
		HANDLE_ERROR;
	}
	...
	while(flag)
	{
		...
		if(FAIL == (clientSockets[socketNum] = accept(serverSocket, (struct sockaddr*)&clientAddr, &clientAddrLen)) && EAGAIN !=errno)
		{
		    if(EINTR == errno && !flag)
	    	{	
				close(serverSocket);
		...
		for(i = 0 ;i < socketNum; ++i)
		{
			...
			if(clientSockets[i])
			{
				if(FAIL == (numOfBytes = recv(clientSockets[i], recMsg, BUFF_SIZE, 0)) && EAGAIN !=errno)
				{
					if(EINTR == errno && !flag)
					{
						close(serverSocket);
