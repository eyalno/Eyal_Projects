...
TreeNode* List2Tree(Node* _list,int _listItemsNum)
{
	...
	if(_listItemsNum == 1)
	{
		_list->m_Next=_list->m_Prev=NULL;			
		return (TreeNode*)_list;
	}

	for (i=0,mid=_list;i<_listItemsNum/2;++i,mid=mid->m_Next);
	
	
	mid->m_Prev=(Node*)List2Tree(_list,_listItemsNum/2);
	mid->m_Next=(Node*)List2Tree(mid->m_Next,_listItemsNum - _listItemsNum/2 -1);
