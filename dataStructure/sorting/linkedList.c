...
ADTERR LLfind(List* _list,int _data)
{
	Node * ptr;
	Node *  
	...
	/*search for items*/
	for (ptr=&_list->m_Head;ptr->m_Data != _num && ptr != &_list->m_Tail; ptr = ptr->m_Next);
	...
	/*optimize for the first item*/
	if(_list->m_Head.m_Next->m_Data==_num) 
	{
		return ERR_ITEM_FOUND;	
	}
